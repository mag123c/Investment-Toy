...
public class MemberController {
    ...
    public ApiResponse<String> signIn(MemberSigninDto memberSigninDto) {
        int signInCheck = memberService.signIn(memberSigninDto);
        try {
            switch(signInCheck) {
                case 0: return ApiResponse.ok("로그인 성공");
                case -1: return ApiResponse.of(HttpStatus.BAD_REQUEST, ("존재하지 않는 ID입니다."), null);
                case -2: return ApiResponse.of(HttpStatus.BAD_REQUEST, ("비밀번호가 일치하지 않습니다."), null);
                default: return ApiResponse.of(HttpStatus.INTERNAL_SERVER_ERROR, "알 수 없는 오류 발생", null);
            }
        } catch (HttpClientErrorException httpClientErrorException) {
            return ApiResponse.of(HttpStatus.BAD_REQUEST, httpClientErrorException.getMessage(), null);
